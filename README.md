...
You have now wired an index view into the routes. Verify it’s working accessing to the following url: `http://localhost:8080/ <http://localhost:8080/>`_
...
If you need another methods you can implement a custom action by decorating the method with the `action` decorator.
...
First at all, we need to define our Viewset in the `views.py` module.
...
And that's it! You can try the defined enpoints whit your favorite Testing API client. 
...
----------------------
webaio handle APIException subclasses, and deal with returning appropriate error responses. This is accompished by adding the webaio middleware `api_exception_handler` to the `aiohttp` app. If you created the project using the `webaio-create-project.py` script, the middleware it is alredy added to the app.
...
We can check the behavior running the tests:
